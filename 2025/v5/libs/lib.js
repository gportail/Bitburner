/** @param {NS} ns */
/**
 * Calcul le nombre de thread max pour le {script} sur le serveur {serverName}
 * @return integer
 */
...
/**
 * Calcul le nombre de thread max pour le {script} sur le serveur {serverName} en fonction de la ram disponible
 * @return integer
 */
export function calcNbThreadPossible(serverName, script, ns) {
  let ram = ns.getScriptRam(script);
  let srvram = ns.getServerMaxRam(serverName) - ns.getServerUsedRam(serverName);
  if (serverName == 'home') srvram -= 32; // on garde 32Go de ram sur home
  return Math.trunc(srvram / ram);
}
/**
 ...
 */
export function calculGainSeconds(ns, host) {
