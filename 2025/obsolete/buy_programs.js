...
import { green } from "libs/colors.js";

/**
 * Affiche l'aide
 */
function help(ns) {
  ns.tprintf(`${yellow}Achète les programmes selon l'argent disponible.`);
  ns.tprintf('options :');
  ns.tprintf('  -h    : aide');
...
export async function main(ns) {
  ...
  for (let prog of progs) {
    if (!ns.fileExists(prog[0], 'home')) {
      if (!ns.hasTorRouter()) {
        l.log(ns, `${green}Il faut acheter le routeur Tor`);
        ns.exit();
      }
      if (ns.getServerMoneyAvailable('home') > prog[1]) {
        l.logf(ns, `${green}Achat du programme %s`, [prog[0]], quiet);
