...
import { calculGainSeconds } from "./lib.js";
...
export function sortServersByMoneyPerSec(ns, sl) {
  if (sl == undefined) sl = servers;
  sl.sort(function (a, b) {
    const a_info = calculGainSeconds(ns, a);
    const b_info = calculGainSeconds(ns, b);
    if (a_info > b_info) return 1;
    if (a_info < b_info) return -1;
    return 0;
  });
 ...
 * @param {NS} ns 
 * @param {String} fromServer Le serveur à partir duquel on recherche les enfants.
 * @returns Array
 */
